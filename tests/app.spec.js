...
test.describe('Match Dashboard', () => {
  test('opens and draws real content on all three charts', async ({ page }) => {
    await signIn(page);
    await seedSquad(page, 16);
    await goToRecordTab(page);

    // Give the charts something to actually plot — a score and a foul.
    await page.click('button[aria-label="Shot from Play — My Team"]');
    await page.click('.outcome-btn.ob-point');
    await page.click('.pp-player:not(.empty)');
    await page.click('#location-modal button:has-text("Skip")');
    await page.click('button[aria-label="Foul Given — Opposition"]');
    await page.click('.outcome-btn:has-text("Cynical")');
    await page.click('.pp-opp');
    await page.click('#location-modal button:has-text("Skip")');

    await page.click('.nav-btn:nth-child(5)'); // Stats tab
    await page.click('.export-row button:has-text("Dashboard")');

    await expect(page.locator('#dashboard-section')).toBeVisible();

    // A canvas can report non-zero pixel dimensions while still being
    // visually empty — checking actual painted pixels is what would have
    // caught the compositing-timing false alarm found while building this.
    const paintedPixels = await page.evaluate((id) => {
